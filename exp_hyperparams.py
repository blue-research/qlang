...
def run_experiment(ansatz):
    ...
    write_log(f"Test Accuracy: {evaluate_model(model, test_circuits, test_labels)}", n_layers, n_single_qubit_params)

# Main program
if __name__ == '__main__':
    ansatz_idx=str(sys.argv[1])
    n_layers=int(sys.argv[2])
    n_single_qubit_params=int(sys.argv[3])
    current_time = sys.argv[4]
    
    ansatzes = {
        "0":IQPAnsatz({AtomicType.NOUN: 1, AtomicType.SENTENCE: 1}, n_layers=n_layers, n_single_qubit_params=n_single_qubit_params),
        "1":StronglyEntanglingAnsatz({AtomicType.NOUN: 1, AtomicType.SENTENCE: 1},n_layers=n_layers, n_single_qubit_params=n_single_qubit_params),
        "2":Sim14Ansatz({AtomicType.NOUN: 1, AtomicType.SENTENCE: 1}, n_layers=n_layers, n_single_qubit_params=n_single_qubit_params),
        "3":Sim15Ansatz({AtomicType.NOUN: 1, AtomicType.SENTENCE: 1}, n_layers=n_layers, n_single_qubit_params=n_single_qubit_params)
    }
    ...
    ansatz=ansatzes[ansatz_idx]
    ansatz_str=ansatzes_str[ansatz_idx]
    print(ansatz_str)
    
    log_dir = os.path.join('logs', current_time)
    os.makedirs(log_dir, exist_ok=True)
    log_dir = os.path.join(log_dir, ansatz_str)
    os.makedirs(log_dir, exist_ok=True)
